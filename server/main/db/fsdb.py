...
class FirestoreDatabase(DatabaseProtocol):
    ...
    def save_user(self, user: User) -> None:
        doc_ref = self._db.collection("users").document(user.firebase_user_id)
        doc_ref.set({"email": user.email})

    def get_algo(self, author_id: UserId, name: str) -> Optional[Algorithm]:
        doc_ref = self._db.collection("algorithms").document(f"{author_id}_{name}")
        doc = doc_ref.get()
        if doc.exists:
            data = doc.to_dict()
    ...
    def save_algo(self, algo: SaveAlgorithmArgs) -> None:
        doc_ref = self._db.collection("algorithms").document(
    ...
    def get_algo_summaries(self, author_id: UserId) -> list[AlgorithmSummary]:
        ...
        for doc in docs:
            data = doc.to_dict()
            summaries.append(AlgorithmSummary(author_email=author_id, name=data["name"]))
        return summaries

    def get_public_algos(self) -> list[ScriptDemoInfo]:
        algos_ref = self._db.collection("algorithms").where("public", "==", True)
        docs = algos_ref.stream()
        public_algos = []
        for doc in docs:
            data = doc.to_dict()
