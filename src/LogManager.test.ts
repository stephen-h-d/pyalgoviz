  ...
  test('getAlgoLogUntilIndex should return concatenated algo logs up to the given index', () => {
    fc.assert(
      fc.property(fc.array(fc.record({
        ...
        viz_log: fc.string(),
        algo_log: fc.string()
      })), fc.nat(), (events: VizEvent[], index: number) => {
        ...
        const expected = events.slice(0, index + 1).map(e => e.algo_log).join('\n');
  ...
  test('getVizLogUntilIndex should return concatenated viz logs up to the given index', () => {
    fc.assert(
      fc.property(fc.array(fc.record({
        ...
        const expected = events.slice(0, index + 1).map(e => e.viz_log).join('\n');
